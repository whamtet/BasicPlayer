...
{
	public static int EXTERNAL_BUFFER_SIZE = 4000 * 40;
	 ...
	 */
	protected void initAudioInputStreamParent() throws BasicPlayerException
	{
		try
		{
			reset();
	...
	public void setDouble(AudioInputStream a, AudioFileFormat b) {
		this.m_audioInputStream = a;
		this.m_audioFileFormat = b;
	}
	public BasicPlayer getThis() {
		return this;
	}
	...
	protected void createLine() throws LineUnavailableException
	{
		...
		if (m_line == null)
		{
			...
			try {
				pipedInputStream = new PipedInputStream(pipedOutputStream, EXTERNAL_BUFFER_SIZE);
			} catch (IOException e) {
			...
			boolean signed = true;//guess
			boolean bigendian = false;
			int bands = 10;//guess
			equalizerInputStream = new EqualizerInputStream(pipedInputStream, samplerate, channels, signed, nSampleSizeInBits, bigendian, bands);
	...
	public IIRControls getEQ() {
		return equalizerInputStream.getControls();
	}
	...
	{
		if (m_status == STOPPED) initAudioInputStreamParent();
		if (m_status == OPENED)
		{
	...
	private class ReadingThread extends Thread {
		public void run() {
			byte[] pcm2 = new byte[EXTERNAL_BUFFER_SIZE];
			while(true) {
				int nBytesRead2;
				try {
					nBytesRead2 = equalizerInputStream.read(pcm2);
//					nBytesRead2 = pipedInputStream.read(pcm2, 0, nBytesRead);
